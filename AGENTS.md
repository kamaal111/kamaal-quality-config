...
- `src/index.ts` builds the config object, not a rule implementation — the actual rules live in
...
- `pnpm build` also runs `scripts/generate-json-config.ts`, which imports the built `dist/index.js`
  ...
  so don't hand-edit `dist/oxlintrc.json` or let the generation script drift from `src/index.ts`'s shape.
